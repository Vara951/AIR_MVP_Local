...
def generate_simple_mvp_data():
    """45 unique incidents - perfect for MVP testing"""

    incidents = []

    # =================================================================
    # PATTERN 1: DATABASE TIMEOUTS (15 incidents, 5 per stack)
    # ROOT CAUSE: Connection pool exhaustion, slow queries, network issues
    # =================================================================
    ...
    # =================================================================
    # PATTERN 2: NULL REFERENCE ERRORS (15 incidents, 5 per stack)
    # ROOT CAUSE: Missing null checks, race conditions, optional fields
    # =================================================================

    null_reference_incidents = [
        # Java Stack
        {
            'id': 'INC-016',
    ...
    # =================================================================
    # PATTERN 3: MEMORY LEAKS (15 incidents, 5 per stack)
    # ROOT CAUSE: Unclosed resources, growing caches, event listeners
    # =================================================================

    memory_leak_incidents = [
        # Java Stack
        {
            'id': 'INC-031',
    ...
    all_incidents = (db_timeout_incidents +
                     null_reference_incidents +
                     memory_leak_incidents)
...
if __name__ == '__main__':
    print("=" * 70)
    print("🚀 GENERATING SIMPLE MVP DATA (45 UNIQUE INCIDENTS)")
    print("=" * 70)

    os.makedirs('data', exist_ok=True)

    incidents = generate_simple_mvp_data()
    ...
    print(f"📂 Saved to: data/incidents.json")

    tech_count = {'java': 0, 'python': 0, 'nodejs': 0}
    error_count = {'TIMEOUT': 0, 'NULL_REFERENCE': 0, 'MEMORY_LEAK': 0}

    for inc in incidents:
        tech_count[inc['tech_stack']] += 1
        error_count[inc['error_type']] += 1

    print("\n📊 Distribution:")
    print(f"   Java: {tech_count['java']} | Python: {tech_count['python']} | Node.js: {tech_count['nodejs']}")
    print(
        f"   Timeouts: {error_count['TIMEOUT']} | Null: {error_count['NULL_REFERENCE']} | Memory: {error_count['MEMORY_LEAK']}")
